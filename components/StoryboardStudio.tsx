...
import { generateStoryboardPlan, generateImage, formatGeminiError } from '../services/geminiService';
...
const StoryboardStudio: React.FC<{
    ...
    const onCreatePlan = async () => {
        ...
        try {
            ...
            setProject(s => ({ ...s, isGeneratingPlan: false, error: formatGeminiError(err) }));
        }
    };
    ...
    const onGenerateGrid = async () => {
        ...
        try {
            ...
            setProject(s => ({ ...s, isGeneratingGrid: false, error: formatGeminiError(err) }));
        }
    };
    ...
    const onGenerateSceneImage = async (sceneId: string) => {
        ...
        try {
            ...
            setProject(s => {
                const next = [...s.scenes];
                next[sceneIdx] = { ...next[sceneIdx], isLoading: false, error: formatGeminiError(err) };
                return { ...s, scenes: next };
            });
        }
    };

...
import { generateImage, editImage, formatGeminiError } from '../services/geminiService';
...
const PhotoshootDirector: React.FC<PhotoshootDirectorProps> = ({ project, setProject }) => {
    ...
    for (const shotType of project.selectedShotTypes) {
      try {
        ...
        setProject(s => ({
          ...s,
          results: s.results.map(r => r.shotType === shotType ? { ...r, image, isLoading: false, error: null } : r),
        }));
      } catch (error: any) {
        ...
        setProject(s => ({
          ...s,
          results: s.results.map(r => r.shotType === shotType ? { ...r, error: errorMsg, isLoading: false } : r),
        }));
      }
    }
  ...
  const handleEditResult = async (index: number, prompt: string) => {
      ...
      try {
          ...
          setProject(s => {
              const newResults = [...s.results];
              newResults[index] = { ...newResults[index], isEditing: false, error: errorMsg };
              return { ...s, results: newResults };
          });
      }
  };

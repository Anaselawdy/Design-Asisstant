...
import { analyzeLogoForBranding, generateImage, formatGeminiError } from '../services/geminiService';
        ...
        try {
            ...
            for (const category of MOCKUP_CATEGORIES) {
                try {
                    ...
                    const image = await generateImage([project.logos[0]], prompt, null);
                    setProject(s => ({
                        ...s,
                        results: s.results.map(r => r.category === category ? { ...r, image, isLoading: false, error: null } : r)
                    }));
                } catch (error: any) {
                    const errorMsg = formatGeminiError(error);
                    setProject(s => ({
                        ...s,
                        results: s.results.map(r => r.category === category ? { ...r, error: errorMsg, isLoading: false } : r)
                    }));
                }
            }
        } catch(err) {
            setProject(s => ({...s, error: formatGeminiError(err), isAnalyzing: false }));
        } finally {

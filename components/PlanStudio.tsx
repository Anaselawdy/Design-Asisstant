...
import { generateCampaignPlan, generateImage, analyzeProductForCampaign, formatGeminiError } from '../services/geminiService';
...
const PlanStudio: React.FC<{
    ...
    const onGenerateIdeaImage = async (ideaId: string) => {
        ...
        try {
            ...
            setProject(s => {
                const next = [...s.ideas];
                next[ideaIdx] = { ...next[ideaIdx], isLoadingImage: false, imageError: formatGeminiError(err) };
                return { ...s, ideas: next };
            });
        }
    };

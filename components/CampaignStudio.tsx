...
import { analyzeProductForCampaign, generateImage, editImage, formatGeminiError } from '../services/geminiService';
...
const CampaignStudio: React.FC<{
    ...
    const onGenerate = useCallback(async () => {
        ...
        try {
            ...
            for (const scenario of scenarios) {
                ...
                try {
                    const image = await generateImage(productImages, prompt, null);
                    setProject(s => {
                        const nextResults = [...s.results];
                        const idx = nextResults.findIndex(r => r && r.scenario === scenario);
                        if (idx !== -1) {
                            nextResults[idx] = { ...nextResults[idx], image, isLoading: false, error: null };
                        }
                    ...
                    setProject(s => {
                        ...
                        const idx = nextResults.findIndex(r => r && r.scenario === scenario);
                        if (idx !== -1) {
                            nextResults[idx] = { ...nextResults[idx], error: errorMsg, isLoading: false };
                        }
                        return { ...s, results: nextResults };
                    });
                }
            ...
            setProject(s => ({ ...s, error: formatGeminiError(err), isGenerating: false }));
        } finally {
    ...
    const handleEditResult = async (index: number, editPromptText: string) => {
        ...
        try {
            ...
            setProject(s => {
                const nextResults = [...s.results];
                nextResults[index] = { ...nextResults[index], isEditing: false, error: errorMsg };
                return { ...s, results: nextResults };
            });
        }
    };
